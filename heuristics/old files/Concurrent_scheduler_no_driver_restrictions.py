...
# Files
shipment_file = constants.shipments_file
...
for cycle in cycles:
    ...
    for i in range(len(cycle) - 3):
        one_waiting_time = start_time_ith_trip_in_cycle(cycle, i + 2) - end_time_ith_trip_in_cycle(cycle, i + 1) - \
                              time_diff_matrix.at[end_loc_ith_trip_in_cycle(cycle, i + 1), start_loc_ith_trip_in_cycle(cycle, i + 2)]
...
for cycle in cycles:
    ...
    # Reliefpoints
    cycle_key = cycle[1]['shipment_id']
    if get_duration_from_cycle(cycle) <= max_duration_reliefpoints and get_driving_time_from_cycle(cycle) <= max_driving_time:
...
# Save the figure
plt.savefig("figures/gantt chart Concurrent Scheduler no driver restrictions.png")
