        ...
        for shipment_tw in self.sorted_shipments_tw:
            ...
            if not placed and len(active_trucks_with_space) > 0:
                ...
                the_best_truck = min(active_trucks_with_space,
                                     key=lambda truck: cost_active_truck(truck,
                                                                         cheapest_compatible_shipment_truck(truck,
                                                                                                            shipment_tw)))
                cost_of_best_truck = cost_active_truck(the_best_truck,
                                                       cheapest_compatible_shipment_truck(the_best_truck, shipment_tw))
                trucks_with_equal_costs = list(filter(lambda truck: cost_active_truck(truck,
                                                                                      cheapest_compatible_shipment_truck(
                                                                                          truck,
                                                                                          shipment_tw)) == cost_of_best_truck,
                ...
                number_of_ties = len(trucks_with_equal_costs)
                # if number_of_ties > 1:
                #     counter += 1
                random_best_truck = trucks_with_equal_costs[random.randint(0, number_of_ties - 1)]  # IMPROVE
                random_best_truck.add_shipment(cheapest_compatible_shipment_truck(random_best_truck, shipment_tw))
                # the_best_trucks = heapq.nsmallest(min(number_of_trucks_with_space, 3),
                #                                   active_trucks_with_space,
                #                                   key=lambda truck: cost_active_truck(truck, cheapest_compatible_shipment_truck(truck, shipment_tw)))
                # random_best_truck = the_best_trucks[min(util.random_0_or_1_or_2(), number_of_trucks_with_space - 1)]
                # random_best_truck.add_shipment(cheapest_compatible_shipment_truck(random_best_truck, shipment_tw))
