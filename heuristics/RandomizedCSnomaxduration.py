...
class RandomizedCSnomaxduration:
    ...
    def get_solution(self):
        ...
        for shipment_tw in self.sorted_shipments_tw:
            ...
            active_compatible_trucks = get_active_compatible(shipment_tw, active_trucks)
            placed = False
            if not placed and len(active_compatible_trucks) > 0:
                number_of_trucks_with_space = len(active_compatible_trucks)
                the_best_trucks = heapq.nsmallest(min(number_of_trucks_with_space, 3),
                                                  active_compatible_trucks,
...
def get_active_compatible(shipment_tw, active_trucks):
