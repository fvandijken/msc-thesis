...
fixed_trucks = cs.get_many_splittable_and_efficient_trucks(number_of_iterations=number_of_iterations_1)
#partial_heur_solution = Schedule(config=config, trucks=trucks)
# toc = time.time()
# print('\n')
...
efficient_trucks = list(filter(lambda truck: truck.get_duration() < truck.get_max_duration(), fixed_trucks))
splittable_trucks = list(filter(lambda truck: truck.is_splittable(), fixed_trucks))
efficient_shipments = 0
for truck in efficient_trucks:
    for ship in truck.shipments:
        efficient_shipments += 1
splittable_shipments = 0
for truck in splittable_trucks:
    for ship in truck.shipments:
        splittable_shipments += 1
#
# print('number of efficient shipments: ', efficient_shipments)
# print('number of splittable shipments: ', splittable_shipments)
...
shipments_to_be_placed = exclude_truck_shipments_from_list(fixed_trucks, input.shipments_tw)
...
random_cs = RandomizedSearchbeforeafter(input=input_second_phase, config=config, input_trucks=efficient_trucks)
solution_1, solution_2, solution_3 = random_cs.get_solution(number_iterations=number_of_iterations_2)
toc = time.time()
time = toc - tic
# print('\n')
# print(greedy_solution_1)
# greedy_solution_1.metrics()
# #shipments_occurance(greedy_solution, input_greedy)
# greedy_solution_1.visualize()
...
final_solution_1.visualize(save=True, with_shipment_ids=False)
final_solution_1.get_depot_distribution()
