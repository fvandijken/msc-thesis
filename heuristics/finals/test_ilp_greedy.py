...
from heuristics.Schedule import Schedule, split_schedule
import check
import time
...
tic = time.time()
mdvsptwilp = MDVSPTWILP(input=input_ilp, config=config)
ilp_solution: Schedule = mdvsptwilp.get_solution()
...
check.shipments_occurance(ilp_solution, input_ilp)
ilp_solution.visualize(save=False, with_shipment_ids=False)
...
feasible_part_ilp_solution.visualize(save=False, with_shipment_ids=False)

split_feasible_ilp_solution = split_schedule(feasible_part_ilp_solution)
# print(split_feasible_ilp_solution)
# print(split_feasible_ilp_solution.metrics())
split_feasible_ilp_solution.visualize(save=False, with_shipment_ids=False)

input_greedy = InputTW(shipments_tw=shipments_to_be_placed)

concurrent_scheduler = RandomizedSearchbeforeafter(input=input_greedy, config=config, input_trucks=[truck for truck in split_feasible_ilp_solution.trucks])
